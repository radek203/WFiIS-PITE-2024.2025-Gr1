...
class App:
    ...
    def create_image_container(self, parent, place_id):
        ...
        if not st.session_state['is_image_generate'][place_id]:
            ...
            prompt, tags = st.session_state['image_generator'].generate_random_prompt(1)
        ...
        raiting = tile.slider("Rating",0,10,key=self.KEY_ID)
        self.KEY_ID+=1
        tile.button("Rate",key=self.KEY_ID,use_container_width=True,on_click=mbc.like_callback, args=(img_data[0], img_data[1],raiting, img_data[2], img_data[3]))
        self.KEY_ID += 1
