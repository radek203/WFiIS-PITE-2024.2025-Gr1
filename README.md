...
- get_top_n_ratings function uses user_encoder.transform, we should check if it is correct, but it should be fine
- There is problem - get_top_n_ratings function uses another categories order than order in saved ratings, (we should sort them??? and check if proper tags combinations (that were rated) are deleted)
- Currently, there is problem with get_top_n_ratings function, all predicted ratings have the same value
